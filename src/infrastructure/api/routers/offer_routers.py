...
def add_offer(request: AddOfferInputDto, session: Session = Depends(get_session)):
    try:
...
def find_offer(offer_id: int, session: Session = Depends(get_session)):
    try:
        offer_repository = OfferRepository(session=session)
        usecase = FindOfferUseCase(offer_repository=offer_repository)
        output = usecase.execute(input=FindOfferInputDto(id=offer_id))
        return output
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except Exception as e:
...
def remove_offer(offer_id: int, session: Session = Depends(get_session)):
    try:
        offer_repository = OfferRepository(session=session)
