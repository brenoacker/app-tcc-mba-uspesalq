...
class FindCartItemsUseCase(UseCaseInterface):
    ...
    async def execute(self, input: FindCartItemsInputDto) -> FindCartItemsOutputDto:
        
        cart = await self.cart_repository.find_cart(cart_id=input.cart_id, user_id=input.user_id)
        if cart is None:
            raise ValueError(f"Cart not found: {input.cart_id}")
        
        cart_items = await self.cart_item_repository.find_items_by_cart_id(cart_id=input.cart_id)
