...
class AddItemUseCase(UseCaseInterface):
    ...
    async def execute(self, user_id: UUID, cart_id: UUID, input: AddItemInputDto) -> AddItemOutputDto:
    
        # verificar se o produto existe
        product_found = await self.product_repository.find_product(product_id=input.product_id)

        if product_found is None:
            raise ValueError(f"Product with code '{input.product_id}' not found")

        # verificar se o produto esta no carrinho
        find_item = await self.cart_item_repository.find_item(item_id=product_found.id)
        
        if find_item:
            usecase = UpdateItemUseCase(cart_item_repository=self.cart_item_repository)
            find_item.quantity += input.quantity
            output = await usecase.execute(cart_item_id=find_item.id, input=UpdateItemInputDto(quantity=find_item.quantity))
